...
def migrate_data_with_progress():
    ...
    files_to_process = [
        ...
        # Economy data files
        {
            'name': 'Khaleej Times Economy',
            'file': 'original_data_backup/khaleej_times_economy_uae_mock_250_with_sentiment.csv',
            'type': 'csv',
            'topic': 'economy',
            'platform': 'Khaleej Times'
        },
        {
            'name': 'LinkedIn Economy',
            'file': 'original_data_backup/uae_linkedin_economic_mock_with_sentiment.csv',
            'type': 'csv',
            'topic': 'economy',
            'platform': 'LinkedIn'
        },
        {
            'name': 'Quora Economy',
            'file': 'original_data_backup/mock_quora_economic_uae_realistic_with_sentiment.csv',
            'type': 'csv',
            'topic': 'economy',
            'platform': 'Quora'
        },
        {
            'name': 'Reddit Economy',
            'file': 'original_data_backup/mock_reddit_economic_uae_realistic_with_sentiment.csv',
            'type': 'csv',
            'topic': 'economy',
            'platform': 'Reddit'
        },
        {
            ...
            'file': 'original_data_backup/reuters_economic_uae_mock_100.csv',
            'type': 'csv',
            'topic': 'economy',
            'platform': 'Reuters'
        }
    ...
    for file_info in files_to_process:
        ...
        try:
            ...
            processed_records = process_platform_data(df, file_info['name'], file_info['topic'], file_info.get('platform'))
...
def process_platform_data(df, platform_name, topic, platform_override=None):
    """Process data based on platform with specific column mapping and original date extraction"""
    
    print(f"🔧 Processing {platform_name} data structure for topic: {topic}...")
    
    try:
        # Handle economy data files with different structure
        if topic == 'economy':
            return process_economy_data(df, platform_name, platform_override)
        
        # Handle education data files (original logic)
...
def process_economy_data(df, platform_name, platform_override):
    """Process economy data files with different column structures"""
    
    print(f"   📝 Processing economy data for {platform_name}...")
    
    try:
        processed_df = pd.DataFrame()
        
        # Use platform override if provided, otherwise extract from platform_name
        platform = platform_override if platform_override else platform_name.replace(' Economy', '')
        
        # Handle different economy data structures
        if 'LinkedIn' in platform_name or platform == 'LinkedIn':
            # LinkedIn economy data has: keyword, post_text, post_date, Comment_text, combined_text, Date, sentiment_predicted
            processed_df['title'] = df.get('post_text', '')
            processed_df['content'] = df.get('post_text', '')
            processed_df['comment'] = df.get('Comment_text', '')
            processed_df['comment_sentiment'] = ''
            processed_df['author'] = ''
            processed_df['url'] = ''
            processed_df['summary'] = df.get('keyword', '')  # Use keyword as summary
            processed_df['sentiment_predicted'] = df.get('sentiment_predicted', '')
            processed_df['sentiment_confidence'] = 0.8  # Default confidence for economy data
            processed_df['relevance_score'] = 0.9  # High relevance for economy data
            processed_df['relevant_to_education_in_uae'] = False  # This is economy, not education
            processed_df['sentiment_negative'] = 0.0
            processed_df['sentiment_neutral'] = 0.0
            processed_df['sentiment_positive'] = 0.0
            processed_df['combined_text'] = df.get('combined_text', '')
            processed_df['platform'] = 'LinkedIn'
            processed_df['topic'] = 'economy'
            
            # Handle dates - use post_date or Date
            if 'post_date' in df.columns:
                processed_df['date'] = df['post_date'].apply(lambda x: parse_date_safely(x, platform_name))
            elif 'Date' in df.columns:
                processed_df['date'] = df['Date'].apply(lambda x: parse_date_safely(x, platform_name))
            else:
                processed_df['date'] = datetime.now().date()
                
        elif 'Khaleej Times' in platform_name or platform == 'Khaleej Times':
            # Khaleej Times economy data has: Title, url, Summary, combined_text, Date, sentiment
            processed_df['title'] = df.get('Title', '')
            processed_df['content'] = df.get('Title', '')  # Use title as content
            processed_df['comment'] = ''
            processed_df['comment_sentiment'] = ''
            processed_df['author'] = ''
            processed_df['url'] = df.get('url', '')
            processed_df['summary'] = df.get('Summary', '')
            processed_df['sentiment_predicted'] = df.get('sentiment', '')
            processed_df['sentiment_confidence'] = 0.8  # Default confidence
            processed_df['relevance_score'] = 0.9  # High relevance for economy data
            processed_df['relevant_to_education_in_uae'] = False  # This is economy, not education
            processed_df['sentiment_negative'] = 0.0
            processed_df['sentiment_neutral'] = 0.0
            ...
            # Generic economy data processing for other platforms
            print(f"   📝 Using generic processing for {platform_name}...")
            
            # Try to map common columns
            processed_df['title'] = df.get('Title', df.get('title', df.get('question', '')))
            processed_df['content'] = df.get('Content', df.get('content', df.get('post_text', '')))
            processed_df['comment'] = df.get('Comment', df.get('comment', df.get('Comment_text', '')))
            processed_df['comment_sentiment'] = ''
            processed_df['author'] = df.get('Author', df.get('author', ''))
            processed_df['url'] = df.get('URL', df.get('url', ''))
            processed_df['summary'] = df.get('Summary', df.get('summary', ''))
            processed_df['sentiment_predicted'] = df.get('sentiment', df.get('sentiment_predicted', ''))
            processed_df['sentiment_confidence'] = 0.8
            processed_df['relevance_score'] = 0.9
            processed_df['relevant_to_education_in_uae'] = False
            processed_df['sentiment_negative'] = 0.0
            processed_df['sentiment_neutral'] = 0.0
            processed_df['sentiment_positive'] = 0.0
            processed_df['combined_text'] = df.get('combined_text', '')
            ...
            # Handle dates
            date_columns = ['Date', 'date', 'post_date', 'created_date']
            date_found = False
            for date_col in date_columns:
                if date_col in df.columns:
                    processed_df['date'] = df[date_col].apply(lambda x: parse_date_safely(x, platform_name))
                    date_found = True
                    break
        ...
        print(f"   ✅ Processed {len(processed_df)} {platform_name} economy records")
        
        return processed_df
        
    except Exception as e:
        print(f"❌ Error processing {platform_name} economy data: {str(e)}")
        return None
        ...
        with db.engine.connect() as conn:
            for _, row in batch_df.iterrows():
                ...
                (title, url, summary, content, comment, comment_sentiment, author, combined_text, relevance_score, 
                 ...
                 sentiment_positive, sentiment_predicted, sentiment_confidence, date, platform, topic)
                ...
                values = {
                    ...
                    'topic': str(row.get('topic', 'education'))
                }
