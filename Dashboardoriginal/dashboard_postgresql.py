...
def load_data_from_database(platform='All Platforms', topic='All Topics'):
    """Load data from PostgreSQL database and clean HTML content"""
    try:
        db_manager = init_database()
        
        # Simple logic: 
        # - If "All Platforms" is selected, show ALL posts from ALL platforms (filtering by topic if specified)
        # - If a specific platform is selected, show posts from that platform (filtering by topic if specified)
        
        if platform == 'All Platforms':
            # Show posts from all platforms, filtered by topic if specified
            df = db_manager.get_platform_data(platform=None, topic=topic)
        else:
            # Specific platform selected - show posts from that platform, filtered by topic if specified
            df = db_manager.get_platform_data(platform, topic=topic)
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        # Topic filter
        topics = ['All Topics', 'Education', 'Economy']
        if 'current_topic' not in st.session_state:
            st.session_state.current_topic = 'All Topics'
        ...
        topic_display = selected_topic if selected_topic != 'All Topics' else 'all topics'
        
        if selected_platform == 'All Platforms':
            st.success(f"📄 Showing {topic_display} posts from ALL platforms")
            
            # Show total count across all platforms and topics
            try:
                db_manager = init_database()
                if selected_topic == 'All Topics':
                    ...
                    total_query = "SELECT COUNT(*) as count FROM social_media_data WHERE topic = %s"
                    result = db_manager.execute_query(total_query, {'topic': selected_topic.lower()})
            ...
            st.success(f"📄 Showing {topic_display} posts from {selected_platform}")
            
            # Show quick stats for the selected platform and topic
            try:
                db_manager = init_database()
                if selected_topic == 'All Topics':
                    platform_query = "SELECT COUNT(*) as count FROM social_media_data WHERE platform = %(platform)s"
                    result = db_manager.execute_query(platform_query, {'platform': selected_platform})
                else:
                    platform_query = "SELECT COUNT(*) as count FROM social_media_data WHERE platform = %(platform)s AND topic = %(topic)s"
                    result = db_manager.execute_query(platform_query, {'platform': selected_platform, 'topic': selected_topic.lower()})
        ...
        if db_connected:
            ...
            if st.button("📊 Show Platform Breakdown"):
                try:
                    db_manager = init_database()
                    stats = db_manager.get_platform_stats(topic=selected_topic)
                    if stats is not None and not stats.empty:
                        st.write(f"**Platform Statistics ({topic_display}):**")
...
def main():
    ...
    if selected_page == "📊 Dashboard":
        ...
        if df.empty:
            topic_msg = f" for topic '{topic}'" if topic != 'All Topics' else ""
            platform_msg = f" from platform '{platform}'" if platform != 'All Platforms' else ""
            st.warning(f"No data found{topic_msg}{platform_msg}. Please run the migration script to import your CSV/Excel data.")
            st.code("python scripts/migrate_data_with_progress.py", language="bash")
            return
        ...
        if topic != 'All Topics' or platform != 'All Platforms':
            filter_info = []
            if topic != 'All Topics':
                filter_info.append(f"Topic: {topic}")
            if platform != 'All Platforms':
                filter_info.append(f"Platform: {platform}")
            st.info(f"📊 Current filters: {', '.join(filter_info)}")
        
        # Render the chatbot interface, passing the current filters for context
        render_chatbot_interface(platform, topic)
